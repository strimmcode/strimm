...
#ifdef UNDER_CE
//=== WAVE FORMAT helpers for Siren and MS-SRAudio
#define WAVE_FORMAT_SIREN 654
#define WAVE_FORMAT_MSSRAUDIO 660

#pragma pack(1)
struct WFEX_MSSRAUDIO : WAVEFORMATEX
{
    /* Base WFEX goes here */
    WORD  wReserved;
    WORD  nDSRPacketSize;
    WORD  nAudioPacketSize;
    /* Additional WFEX goes here */
};

struct WFEX_SIREN : WAVEFORMATEX
{
    /* Base WFEX goes here */
    WORD wSiren7FrameSize;
};

struct WFEX_MSSRAUDIO_SIREN : WFEX_MSSRAUDIO
{
    /* Base WFEX goes here */
    WFEX_SIREN wfexSiren;
};
#pragma pack()
#endif
...
#ifdef UNDER_CE
    else if (SPSF_MSSRA_SirenSR == eFormat)
    {
        int NumBytes = sizeof( WFEX_MSSRAUDIO_SIREN );
        WFEX_MSSRAUDIO_SIREN * pwfex = (WFEX_MSSRAUDIO_SIREN *)::CoTaskMemAlloc( NumBytes );
        *ppCoMemWaveFormatEx = pwfex;
        if( pwfex )
        {
            memset( pwfex, 0, NumBytes );
            pwfex->wFormatTag       = WAVE_FORMAT_MSSRAUDIO;
            pwfex->nChannels        = 1;
            pwfex->nSamplesPerSec   = 16000;
            pwfex->nAvgBytesPerSec  = 2800;
            pwfex->nBlockAlign      = 56;
            pwfex->wBitsPerSample   = 0;
            pwfex->cbSize           = 26;
            pwfex->wReserved        = 0;
            pwfex->nDSRPacketSize   = 15;
            pwfex->nAudioPacketSize = 40;
            pwfex->wfexSiren.wFormatTag       = WAVE_FORMAT_SIREN;
            pwfex->wfexSiren.nChannels        = 1;
            pwfex->wfexSiren.nSamplesPerSec   = 16000;
            pwfex->wfexSiren.nAvgBytesPerSec  = 2000;
            pwfex->wfexSiren.nBlockAlign      = 40;
            pwfex->wfexSiren.wBitsPerSample   = 0;
            pwfex->wfexSiren.cbSize           = 2;
            pwfex->wfexSiren.wSiren7FrameSize = 320;            
...
public:
...
#ifdef UNDER_CE
          case WAVE_FORMAT_MSSRAUDIO:
          {
            // Only check for an exact match for SirenSR. We can generalize this if we add additional formats.
            if (m_pCoMemWaveFormatEx->cbSize >= sizeof(WFEX_MSSRAUDIO_SIREN) - sizeof(WAVEFORMATEX))
            {
                WFEX_MSSRAUDIO_SIREN* pWfex = (WFEX_MSSRAUDIO_SIREN*)m_pCoMemWaveFormatEx;
                if (WAVE_FORMAT_SIREN == pWfex->wfexSiren.wFormatTag)
                {
                    return SPSF_MSSRA_SirenSR;
                }
            }
    ...
    ULONG Serialize(__out_bcount(cbBuffer) BYTE *pBuffer, __in ULONG cbBuffer) const
    {
