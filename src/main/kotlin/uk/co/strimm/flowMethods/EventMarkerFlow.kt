...
class EventMarkerFlow : FlowMethod {
    ...
    override fun init(flow: Flow) {
        ...
        /**
         * This is so the the markers generated by this flow can be associated with the original data. This is so that
         * when loading a previous experiment, the data can be linked and an event marker ticker can be added to the
         * correct plot. This is entirely optional, hence the checking for non-empty strings
         */
        if(this.flow.associatedSaveSink != "") {
            val downStreamSaveSinks = GUIMain.experimentService.getDownstreamSaveSinksFromFlow(this.flow.flowName)
            if(downStreamSaveSinks.isNotEmpty()) {
                //Assumption that there is a one-to-one relationship between a flow and a save sink
                GUIMain.actorService.fileManagerActor.tell(
                    TellMarkerFlowMap(
                        markerFlowAssociation = Pair(downStreamSaveSinks.first(), this.flow.associatedSaveSink)), null)
            }
        }
    }
