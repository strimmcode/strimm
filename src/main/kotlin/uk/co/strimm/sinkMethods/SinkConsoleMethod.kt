...
class SinkConsoleMethod() : SinkMethod {
    ...
    override fun run(data : List<STRIMMBuffer>){
                ...
                for (f in 0..data.size-1){
                    val im = data[f] as STRIMMSignalBuffer
                    val data = im.data
                    val times = im.times
                    var cnt = 0
                    for (ff in 0..im.numSamples-1){
                        print(times!!.get(ff).toString() + ":" )
                        for (fff in 0..im.channelNames!!.size-1){
                            print((data!!.get(cnt)).toString() + " ")
                        }
                        println("")
                    }
                }
