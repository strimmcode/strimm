...
//    private val logDirectory = "./log"
    ...
    init {
        ...
        val fileName = "./log" + "/" + date.toString() + "T" + time.withNano(0).toString().replace(":".toRegex(), "-") + ".log"
    ...
    private fun clearLogFiles(){
        ...
        if (numLogFiles > logFileLimit) {
            ...
            while (numLogFiles > logFileLimit) {
                try {
                    Files.deleteIfExists(Paths.get("./log" + "/" + justTextFiles[fileCounter]))
                    fileCounter++
                    numLogFiles--
