...
class StrimmActor(actorMap : HashMap<Class<out Any>, Class<out ActorMessage>>) : AbstractActor(){
    ...
    override fun createReceive(): Receive {
        ...
        .match<CreateFileManagerActor>(CreateFileManagerActor::class.java){
                message -> createFileManagerActor(message)
                sender().tell(Acknowledgement.INSTANCE, self())
            }
            .match<CreateDatasetActor>(CreateDatasetActor::class.java){
                message -> createDatasetActor(message)
                sender().tell(Acknowledgement.INSTANCE, self())
            }
    ...
    private fun createDatasetActor(create : CreateDatasetActor){
        val name = create.content as String
        val actorName = GUIMain.actorService.makeActorName(name)
        val datasetActor = GUIMain.actorService.createActor(DatasetActor.props(), actorName, DatasetActor::class.java)
        GUIMain.loggerService.log(Level.INFO,"Created dataset actor $actorName")
        datasetActor.tell(Message(name),self)

    }
    private fun createFileManagerActor(create : CreateFileManagerActor){
        val name = create.content as String
        val actorName = GUIMain.actorService.makeActorName(name)
        val fileManagerActor = GUIMain.actorService.createActor(FileManagerActor.props(), actorName, FileManagerActor::class.java)
        GUIMain.loggerService.log(Level.INFO,"Created filemanager actor $actorName")
        //
        GUIMain.actorService.fileManagerActor = fileManagerActor
        fileManagerActor.tell(Message(name),self)
