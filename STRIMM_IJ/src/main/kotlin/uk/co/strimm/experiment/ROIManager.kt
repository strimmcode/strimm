...
class ROIManager{
    companion object {
        ...
        fun createROIObjectFromOverlay(overlay : Overlay, overlayBaseName : String, deviceLabel : String, flowName: String) : ROI{
            ...
            when(overlay){
                is EllipseOverlay -> {
                    roiToReturn.ROItype = ExperimentConstants.ConfigurationProperties.ROI_ELLIPSE_TYPE
                ...
                is RectangleOverlay -> {
                    roiToReturn.ROItype = ExperimentConstants.ConfigurationProperties.ROI_RECTANGLE_TYPE
                    roiToReturn.x = overlay.getOrigin(0)
                    roiToReturn.y = overlay.getOrigin(1)
        ...
        fun createOverlayFromROIObject(roi: ROI) : Overlay?{
            when {
                roi.ROItype == ExperimentConstants.ConfigurationProperties.ROI_RECTANGLE_TYPE -> {
                ...
                roi.ROItype == ExperimentConstants.ConfigurationProperties.ROI_ELLIPSE_TYPE -> {
                ...
                roi.ROItype == ExperimentConstants.ConfigurationProperties.ROI_POLYGON_TYPE -> {
                    //TODO
        ...
        /**
         * When an overlay has been created, register this so the corresponding actors can route the data properly
         * @param overlay The ImageJ overlay created from a STRIMM experiment ROI
         * @param associatedActor Every overlay should be associated with a camera actor (i.e. where it came from)
         */
        fun registerRoutedROI(overlay : Overlay, associatedActor : ActorRef, flowName : String){
            GUIMain.actorService.routedRoiList[overlay] = Pair(associatedActor, flowName)
        }
    }
}